...
class Client(object):
    def __init__(self,H=None,p=None,i=None,e=None,t=None,s=None,d=None,vision=False):
        ...
        self.debug= False
        self.maxSteps= 200  # 50steps/second
...
import pandas as pd
def export_to_csv(dict, name):
    import json
    with open('data.json', 'w') as outfile:
        json.dump(dict, outfile)
    pdd = pd.DataFrame(dict).transpose()
    pdd = pdd.drop(columns=['opponents', 'damage', 'racePos', 'track', 'fuel','gear'
                            ,'focus', 'lastLapTime', 'wheelSpinVel', 'z'])
    pdd.to_csv(name + ".csv")
...
def drive_example(c):
    ...
    # Write data
    data_to_write = c.S.d.copy()
    data_to_write[u'steer'] = R[u'steer']
    global step
    c.car_data[str(step)] = data_to_write
    step+=1
...
if __name__ == u"__main__":
    ...
    for step in xrange(C.maxSteps,0,-1):
        ...
        drive_example(C)
        C.respond_to_server()
    export_to_csv(C.car_data, u"race.csv")
    C.shutdown()
